...
[![Feedme](https://raw.githubusercontent.com/aarong/feedme-client/master/logo.svg)](https://feedme.global)
...
- `err.name === "INVALID_MESSAGE: ..."` - The server transmitted a message that
  was not valid JSON or that violated one of the JSON schemas laid out in the
  specification.
...
- `err.name === "UNEXPECTED_MESSAGE: ..."` - The server transmitted a message
  that was invalid given the state of the conversation.

  - `err.serverMessage` (object) contains the server message.

- `err.name === "INVALID_DELTA: ..."` - The server transmitted a feed delta that
  was invalid given the state of the feed data.

  - `err.serverMessage` (object) contains the server message.

  - `err.deltaError` (object) contains the error generated by the delta.

- `err.name === "INVALID_HASH: ..."` - The feed data hash transmitted by the
...
- `close`
  ...
  - `err.name === "BAD_ACTION_REVELATION: ..."`
