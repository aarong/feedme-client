...
(async () => {
  ...
  const config = {
    ...
    specDir: "./tests",
    specFiles: ["tests/**/*.test.*js"], // mjs and js

    // Dependencies of the client are included in the browser bundle
    ...
    // and NPM packages are generally still CJS
    ...
    browser: {
      ...
      remoteSeleniumGrid: {
        ...
        "sauce:options": {
          tunnelIdentifier: "feedme-client-tunnel", // Tunnel name, not id
          username: process.env.SAUCE_USERNAME, // Documented incorrectly with camel case https://github.com/jasmine/jasmine-browser-runner
  ...
  // Run the tests in Jasmine browser runner
  console.log("Launching tests in Jasmine browser runner...");
  const result = await jasmineBrowser.runSpecs(config);
